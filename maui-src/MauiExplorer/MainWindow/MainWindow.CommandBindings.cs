...
{   
    ...
    public partial class MainPage : ContentPage, IFlyoutProvider, IPushApplicationEvent, IMainWindow, IExecuteMainCommand
    {
            ...
            AasxMenuActionTicket ticket)
        {
            ...
            if (cmd == "editkey")
                _viewModel.MainMenu?.SetChecked("EditMenu", _viewModel.MainMenu?.IsChecked("EditMenu") != true);

            if (cmd == "hintskey")
                _viewModel.MainMenu?.SetChecked("HintsMenu", _viewModel.MainMenu?.IsChecked("HintsMenu") != true);

            if (cmd == "showirikey")
                _viewModel.MainMenu?.SetChecked("ShowIriMenu", _viewModel.MainMenu?.IsChecked("ShowIriMenu") != true);
                ...
                || cmd == "checksmtelements")
            {
                ...
                {
                    _viewModel.MainMenu?.SetChecked("EditMenu", editMode);
                }

                if (ticket.ScriptMode && cmd == "hintsmenu" && ticket["Mode"] is bool hintsMode)
                {
                    _viewModel.MainMenu?.SetChecked("HintsMenu", hintsMode);
                }
            ...
            if (cmd == "test")
            {
                ...
                // do
                Log.Singleton.Info(StoredPrint.Color.Blue, "Blue");
            ...
            if (cmd == "eventsshowlogkey")
                _viewModel.MainMenu?.SetChecked("EventsShowLogMenu", _viewModel.MainMenu?.IsChecked("EventsShowLogMenu") != true);
        ...
        {
            return _viewModel.MainMenu?.IsChecked("EventsShowLogMenu") == true;
        }
            ...
            AasxMenuActionTicket ticket)
        {
            ...
            try
            {
                if (Options.Curr.UseFlyovers) this.StartFlyoverAsync(new EmptyFlyoutPage());
#if TODO_IMPORTANT
                AasxPrintFunctions.PrintSingleAssetCodeSheet(ticket.AssetInfo.GlobalAssetId, ticket.AAS.IdShort);
#endif
                if (Options.Curr.UseFlyovers) this.CloseFlyoverAsync();
            }
            catch (Exception ex)
            {
        ...
        public async void CommandBinding_ConnectRest()
        {
            await Task.Yield();
#if TODO_IMPORTANT
            var uc = new TextBoxFlyoutPage("REST server adress:", AnyUiMessageBoxImage.Question);
            if (lastConnectInput == "")
            {
