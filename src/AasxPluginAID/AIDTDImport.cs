...
namespace AasxPluginAID
{
    class AIDTDImport
    {
        ...
        public static ISubmodelElement BuildAasElement(JObject tdJObject, JObject tdSchemaObject)
        {
            ...
            {
                if (formText.Contains("_"))
                {
                    formText = formText.Replace("_", ":");
                }
            ...
            else if (elemType == "SubmodelElementCollection")
            {
                ...
                //This form conversion is at the moment not in AID V1.0 specification. might be used in later version
                ...
                else if (formText == "forms")
                {
                    ...
                    foreach (var _form in tdJObject["forms"])
                    {
                        ...
                        {
                            foreach(var _formSchemaJObject in tdSchemaObject["childs"])
                            {
                                JObject formSchemaJObjects = JObject.FromObject(_formSchemaJObject);
                                if(formJObject.ContainsKey(formSchemaJObjects["tdText"].ToString()))
                                {
                                    ISubmodelElement formElement = BuildAasElement(formJObject, formSchemaJObjects);
                                    if (formElement != null)
                                    {
                                        submodelElement.Add(formElement);
                                    }
                                }
        ...
        {   
            ...
            try
            {
                foreach (var childElem in interfaceJObject["childs"])
                {
                    ...
                    else if(tdJObject.ContainsKey(childSchemaObject["formtext"].ToString()))
